...
- Type
- Constraints (e.g., `occasion_tag` enum values: `'solo'`, `'date'`, `'business'`, `'group'`, `'comfort'`)
- Default value
...
- State the exact function signature: `compute_similarity(a: TasteProfile, b: TasteProfile, context_id: str | None) -> float`
...
Define the exact JSON schema for `RankedItemInput`, `Recommendation`, and `ClusterResult`.
...
│   ├── test_clustering.py   # HDBSAN integration with synthetic data.
...
Strict rule: `main.py` has no business logic. `models.py` has no DB logic. Enforce boundary violations explicitly.
...
*Deliverable:* `src/models.py` and `tests/test_models.py`. Must validate backward compat migration path (if any).
...
*Deliverable:* `src/main.py` with all routes wired to Phase 2 and 3. `tests/test_api.py` with 100% route coverage.
...
|---|---|
| AI defaults to K-Means because "clustering" usually means K-Means. | Explicitly name HDBSCAN in title. Provide `pip install` command in stack lock. |
| AI stores `rank` as raw int because the old models did. | Pillar 2 rewrite: `rank` is `@computed_field`. Show derivation logic. |
| AI guesses folder structure and dumps logic in `main.py`. | Chapter 5 file tree is canonical. Module boundary rules are enforceable. |
| AI suggests pg_trgm/Elasticsearch for search. | API spec says exact `LIKE` search. No ambiguity. |
| AI builds a React frontend because the old TDD mentioned Next.js. | Redline 5: Frontend is out of scope for Phases 1-5. |
