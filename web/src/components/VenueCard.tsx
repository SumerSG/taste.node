...
function tierBadge(n: number | null) {
  if (!n) return "bg-cream text-ink-faint";
  const colors = [
    ...
    <div className="taste-card group flex flex-col gap-3 p-0">
      <div className="relative aspect-[16/10] overflow-hidden cursor-pointer" onClick={onClick}>
        ...
        <div className="absolute left-3 top-3 flex flex-wrap gap-1.5">
          {venue.price_tier && (
            <span className={`rounded-lg px-2 py-0.5 text-xs font-bold ${tierBadge(venue.price_tier)}`}>
              {tierLabel(venue.price_tier)}
            </span>
          )}
        ...
        {score !== undefined && (
          <div className="absolute right-3 top-3 rounded-full bg-sienna-500 px-2.5 py-1 text-xs font-bold text-white shadow-md">
      ...
      <div className="flex flex-1 flex-col gap-2 px-4 pb-4 pt-1">
        <div className="flex items-start justify-between gap-2">
          <div>
            <h3 className="font-serif text-lg text-ink">{venue.name}</h3>
            <p className="text-sm text-ink-faint">{venue.cuisines.join(" · ")}</p>
          </div>
        </div>
        ...
        {explanation && (
          <p className="mt-1 text-sm italic text-ink-muted">“{explanation}”</p>
        )}
